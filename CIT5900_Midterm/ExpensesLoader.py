        ...
        with open(file, 'r') as f:
            for line in f:
                ...
                if line:  #ignoring blank lines
                    splitted = line.split(':')
                    if len(splitted) == 2: #checking if there are are two fields
                        expense_type = splitted[0].strip() #removinf whitespaces from the splitted part
                        try:
                            amount = float(splitted[1].strip()) #casting the amount after removing whitespaces
                            if expense_type in expenses:
                                expenses[expense_type].add_amount(amount) #add to existing entry
                            else:
                                expenses[expense_type] = Expense(expense_type, amount) #create a new entry
                        except ValueError:
                            #ignore lines with non-numeric amounts
                            pass
