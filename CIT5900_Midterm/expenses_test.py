...
class Expenses_Test(unittest.TestCase):
    ...
    def test_get_expense(self):
        ...
        # test getting expenses with case-sensitive expense type (non-existing)
        self.assertIsNone(expenses_manager.get_expense(self.expenses, "Coffee"))

        # test getting expenses with another expense type (existing)
    ...
    def test_add_expense(self):
        ...
        # test adding to an existing expense
        expenses_manager.add_expense(self.expenses, "coffee", 5.0)
        self.assertAlmostEqual(17.40, self.expenses.get("coffee").amount)

        expenses_manager.add_expense(self.expenses, "family", 2.55)
        self.assertAlmostEqual(35.0, self.expenses.get("family").amount)

        # test adding to an non-existing expense 
        expenses_manager.add_expense(self.expenses, "Coffee", 10.0)
        self.assertAlmostEqual(10.0, self.expenses.get("Coffee").amount)
    ...
    def test_deduct_expense(self):
        ...
        with self.assertRaises(RuntimeError):
            expenses_manager.deduct_expense(self.expenses, "coffee", 15.0)

        # test deducting from non-existing expense
        with self.assertRaises(RuntimeError):
            expenses_manager.deduct_expense(self.expenses, "micellaneous", 10.0)
    ...
    def test_update_expense(self):
        ...
        #   Test updating a non-existing expense
         ...
         #updating a non-existing expense test
        self.assertIsNone(expenses_manager.update_expense(self.expenses, "new", 69.99))

        # another existing expense update test
        expenses_manager.update_expense(self.expenses, "coffee", 21.99)
        self.assertAlmostEqual(21.99, expenses_manager.get_expense(self.expenses, "coffee").amount)
    ...
    def test_export_expenses(self):
        ...
        file_non_existent = 'export_non_existent.txt'
        expense_types_non_existent = ['electricty', 'internet']
        expenses_manager.export_expenses(self.expenses, expense_types_non_existent, file_non_existent)

        # read the exported file with non-existent types
        f_non_existent = open(file_non_existent)
        lines_non_existent = f_non_existent.readlines()
        f_non_existent.close()

        # ensure the exported file with non-existent types is empty because nothing was written
        self.assertEqual(0, len(lines_non_existent))
