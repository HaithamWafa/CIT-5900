...
class ExpensesManager(object):
    ...
    def add_expense(self, expenses, expense_type, value):
        ...
        if expense_type in expenses:
            ...
            new_expense = Expense(expense_type, value)
            expenses[expense_type] = new_expense
    ...
    def deduct_expense(self, expenses, expense_type, value):
        ...
        if expense_type not in expenses:
            ...
            raise RuntimeError(f"The expense type '{expense_type}' does not exist.")
        else:

            expense = expenses[expense_type]
            amount = expense.amount
            if value > amount:
                raise RuntimeError("Deduction value is greater than the existing amount of the expense type.")
            else:
    ...
    def sort_expenses(self, expenses, sorting):
        ...
        """
        expenses_list = [(expense.expense_type, expense.amount) for expense in expenses.values()]

        if sorting == 'expense_type':
            sorted_expenses = sorted(expenses_list, key=lambda x: x[0])
        elif sorting == 'amount':
            sorted_expenses = sorted(expenses_list, key=lambda x: x[1], reverse=True)
        else:
    ...
    def export_expenses(self, expenses, expense_types, file):
        ...
        with open(file, 'w') as f:
            for expense_type in expense_types:
                if expense_type in expenses: #skipping if doesnt exist
